...
bool initArray(int n)
{
	...
	{
	case 1:
		return inputArray("归并排序并行测试1.txt");
		break;
	case 2:
		return inputArray("归并排序并行测试2.txt");
		break;
	case 3:
		return inputArray("归并排序并行测试3.txt");
		break;
	default:
		return false;
	}
...
#pragma omp parallel \
	default(none) shared(k_route_matrix,leftNumbers,result,rows,leftSize,k) private(row_count,k_count)
	{
#pragma omp for
...
#pragma omp parallel \
	default(none) shared(k_route_matrix,row,column)
	{
		// ----
#pragma omp for 
		for (int i = 0; i < row; ++i)
...
#pragma omp for
	...
	{
#pragma omp parallel \
	default(none) shared(k_route_matrix,column,row,thread,i) private(count)
	{
		count = 1;
#pragma omp for schedule(static,(column + row - 1)/thread)
	...
	{
#pragma omp parallel \
	default(none) shared(k_route_matrix,slopeTimes,column,row,thread,i) private(count)
	{
		count = 1;
#pragma omp for schedule(static,(column + row - 1)/thread)
...
#pragma omp parallel \
	default(none) shared(k_route_matrix,iter,result,row,column,leftNumbers) 
#pragma omp for
	for (int i = 0; i < row; ++i)
	{
		for (int j = 0; j < column; ++j)
