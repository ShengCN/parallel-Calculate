...
int main(int argc, char const *argv[])
{
	...
	for (double t=T; terminal_T<t; t*=k)
	{
		...
		{		
			...
			for (int it = omp_get_thread_num()*(iter/num_threads); it < (omp_get_thread_num()+1)*(iter/num_threads); ++it)
			{
				// cout<<it<<" ";
		...
		// 线程中最小的和原先最小的path比较
		if (S_path_list[min_index]<S_path)
		{
			S_path = S_path_list[min_index];
			for (int i = 0; i < data_col; i++)
			{
				S_ID[i] = S_ID_list[min_index][i];
			}
		}
